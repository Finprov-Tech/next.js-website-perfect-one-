...
function fromCmsTestimonial(t: CMSTestimonial): StudentTestimonial {
  return {
    id: String(t.id),
    name: t.name,
    program: t.program,
...
export function TestimonialsPageClient({ cmsPage }: { cmsPage: CMSPage | null }) {
  ...
  const heroBadge = banner?.badge_text || "Learner Reviews";
  const heroHeading = banner?.heading || "Hear What Our Students Have To Say About Us!";
  const heroParagraph =
    banner?.paragraph ||
  ...
  const initialTestimonials: StudentTestimonial[] = cmsPage?.testimonials?.length
    ? cmsPage.testimonials.map(fromCmsTestimonial)
    : studentTestimonials;

  const [testimonialsList, setTestimonialsList] = useState<StudentTestimonial[]>(initialTestimonials);

  const pageSchema = generateSchemaForPage(cmsPage, `${SITE_URL}/testimonials/`) ?? organizationSchema;
          ...
          <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr] lg:items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-gold/40 bg-gold/10 px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-gold">
                <MessageSquare className="h-4 w-4" /> {heroBadge}
              </div>
